...
#define GetPlayerNameEx(%0)		pInfo[%0][pName] 			// имя игрока
#define GetPlayerAccountID(%0)	pInfo[%0][pID] 	// ид аккаунта
#define IsPlayerLogged(%0)		pInfo[%0][pLogged]		// статус авторизации
// == == == == [ Дефайны ] == == == ==
	...
	switch(dialogid)
	{
		...
		case 13:
		{
			...
			if(response)
			{
				...
				mysql_format(dbHandle, string, sizeof(string), fmt_msg, money, amount, pInfo[playerid][pID]);
...
public PlayerLogin(playerid)
{
	...
	if(rows)
	{
		...
		pInfo[playerid][pCredit] = cache_get_field_content_int(0, "Credit");
